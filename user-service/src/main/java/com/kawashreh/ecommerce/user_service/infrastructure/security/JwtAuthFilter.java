 ...
 * entirely, since only the api-gateway validated tokens. /register and /login stay
 * public since they are how a token is obtained in the first place.
 * <p>
 * Once a token validates, this filter overwrites the X-User-ID/X-User-Name/
 * X-User-Role headers on the request with values derived from the token's verified
 * claims. This is the fix for GH #19: ownership checks in UserServiceImpl and
 * AddressServiceImpl read X-User-ID via {@code @RequestHeader}, and previously that
 * header was whatever the gateway (or a direct caller bypassing it) set - now it can
 * only ever be the id embedded in a validly signed token. RoleController (GH #18)
 * uses the verified X-User-Role header to gate create/delete to ADMIN.
 */
@Component

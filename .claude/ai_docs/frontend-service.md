...
| `AddressServiceClient.getAddressById` | `GET /api/v1/address/{id}` | user-service | `ProfileController.addressModal` — found during GH #64 review, not the original issue: user-service previously had no `X-User-ID` scoping on this endpoint at all, so any authenticated user could load any other user's address (street/phone/etc.) via this modal by supplying an arbitrary `id` query param. Now scoped server-side the same way `getAddresses`/`searchAddresses` are; no client change needed, same reasoning |

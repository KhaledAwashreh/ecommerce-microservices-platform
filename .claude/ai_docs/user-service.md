...
| GET | `/{addressId}` | path `addressId`, header `X-User-ID: UUID` (required) | `CreateAddressResponse` or empty | 200 / 404 / **403** | Found during GH #64 review (not the original issue): previously took no header and no ownership check at all — any authenticated user could read any other user's address by UUID, exploitable live via the frontend's address-edit modal. Now scoped like PUT/DELETE: `AddressServiceImpl.find` checks the address's owning user equals `X-User-ID`, else `ForbiddenException` (→403). No longer `@Cacheable` — caching by `#id` alone would have skipped the ownership check on a cache hit. |

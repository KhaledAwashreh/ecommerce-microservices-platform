...
| `constants/JwtConstants.java` | Shared JWT secret + expiration. Only in `api-gateway` and `user-service` — the two services that actually parse/sign tokens in Java. | `user-service/src/main/java/.../constants/JwtConstants.java` |
...
| `frontend-service` | Declares `jwt.secret`/`jwt.expiration` in `application.yml` with no corresponding Java code found that reads it — likely dead config. | `frontend-service/src/main/resources/application.yml:17-19` |
...
| all backend services | JWT secret (`JwtConstants.SECRET`) is a hardcoded string literal, identical across `api-gateway`, `user-service`, and `frontend-service`'s YAML — not sourced from env/secret store. | `user-service/src/main/java/.../constants/JwtConstants.java:7`, `api-gateway/src/main/java/.../constants/JwtConstants.java:7` |
